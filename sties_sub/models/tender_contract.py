...
class TenderContract(models.Model):
    ...
    lead_id = fields.Many2one('crm.lead.tender', string='Appel d\'offre', readonly=True, track_visibility='onchange',
                              track_sequence=1, index=True)
    ...
    amount_total_contract_invoiced = fields.Monetary(string='Total facturé ', store=False, readonly=True,
                                                     compute='_amount_all_contract_values')
    ...
    amount_total_hors_contract_to_invoice = fields.Monetary(string='Total à facturer ', store=False, readonly=True,
                                                            compute='_amount_all_contract_values')
    amount_total_hors_contract_invoiced = fields.Monetary(string='Total facturé ', store=False, readonly=True,
                                                          compute='_amount_all_contract_values')
    ...
    amount_total_to_invoice = fields.Monetary(string='Total à facturer ', store=False, readonly=True,
                                              compute='_amount_all_contract_values')
    amount_total_invoiced = fields.Monetary(string='Total facturé ', store=False, readonly=True,
                                            compute='_amount_all_contract_values')
    ...
    @api.depends('contract_lines.qty_delivered', 'contract_lines.qty_invoiced', 'contract_extra_lines',
                 'contract_lines.subtotal_invoiced', 'contract_lines.subtotal_to_invoice')
        ...
        for contract in self:
            ...
            contract.update({
                ...
                'percentage_of_target': 100 * (
                            amount_total_invoiced - amount_total_invoiced_hors + amount_total_to_invoice) / contract.amount_total if contract.amount_total > 0.0 else 0.0,
                ...
                'global_percentage_of_target': 100 * (
                            amount_total_to_invoice_hors + amount_total_to_invoice + amount_total_invoiced) / contract.amount_total if contract.amount_total > 0.0 else 0.0
...
class ContractLines(models.Model):
    ...
    def _get_subtotal_values(self):
        for contract_line in self:
            ...
            contract_line.percentage_of_target = 100 * (
                        amount_taxexcl_invoiced + amount_taxexcl_to_invoice) / contract_line.price_subtotal if contract_line.price_subtotal > 0.0 else 0.0
    ...
    product_uom_qty = fields.Float(string='Qté du contrat', digits=dp.get_precision('Product Unit of Measure'),
